...
npm audit`,
  ...
  // ── Reading List ──
  const readingColl = await prisma.collection.create({
    data: {
      name: "Reading List",
      description: "Articles, books and resources to read",
      defaultTypeId: typeMap.get("link")!,
      userId: user.id,
    },
  });
  ...
  const readingLinks = [
    {
      ...
      description: "Summary of Clean Code by Robert C. Martin",
      url: "https://gist.github.com/wojteklu/73c6914cc446146b8b533c0988cf1d29",
    },
    {
      title: "You Don't Know JS Yet",
      description: "Book series by Kyle Simpson on JavaScript fundamentals",
      url: "https://github.com/getify/You-Dont-Know-JS",
    },
  ];

  for (const r of readingLinks) {
    const item = await prisma.item.create({
      data: {
        title: r.title,
        contentType: "text",
        ...
        userId: user.id,
        itemTypeId: typeMap.get("link")!,
        collections: { create: { collectionId: readingColl.id } },
      },
    });
    console.log(`   - "${item.title}" (link)`);
  }
  ...
  // ── Quick Notes ──
  const notesColl = await prisma.collection.create({
    data: {
      name: "Quick Notes",
      description: "Random notes and ideas on software engineering",
      defaultTypeId: typeMap.get("note")!,
      userId: user.id,
    },
  });
...
### Jotai
- Atomic approach, similar to Recoil
- Great for fine-grained reactivity
- No store needed, atoms are standalone

### Redux Toolkit
- Most mature ecosystem
- Best for large enterprise apps
- Heavier boilerplate but excellent devtools`,
    },
    {
      title: "Architecture Decision Record",
      description: "ADR template for documenting architectural decisions",
      language: "markdown",
      content: `# ADR-001: Choose Database Technology

## Status
Accepted (2026-01-15)

## Context
We need a primary datastore that supports:
...
## Decision
Use PostgreSQL with Neon for serverless deployment.
  ...
  for (const n of notes) {
    const item = await prisma.item.create({
      data: {
        ...
        isFavorite: n.title === "State Management Comparison",
        userId: user.id,
        itemTypeId: typeMap.get("note")!,
        collections: { create: { collectionId: notesColl.id } },
      },
    });
    console.log(`   - "${item.title}" (note)`);
  }

  // ── Learning Resources ──
  const learningColl = await prisma.collection.create({
    data: {
      name: "Learning Resources",
      description: "Curated learning materials for developers",
      defaultTypeId: typeMap.get("link")!,
      userId: user.id,
    },
  });
  ...
  const learningLinks = [
    {
      ...
      description: "Learn system design fundamentals with examples",
    ...
    },
    {
      ...
      description: "Step-by-step backend development roadmap",
      url: "https://roadmap.sh/backend",
    },
  ];

  for (const l of learningLinks) {
    const item = await prisma.item.create({
      data: {
        title: l.title,
        contentType: "text",
        ...
        url: l.url,
        description: l.description,
        isPinned: l.title === "System Design Primer",
        userId: user.id,
        itemTypeId: typeMap.get("link")!,
        collections: { create: { collectionId: learningColl.id } },
      },
    });
    console.log(`   - "${item.title}" (link)`);
  }
